...
then
	 vultr-cli server list | sed -n '/^===/q;p'	
fi

if [[ ${action} == 'instance-destroy' ]]
then
	instance_list=$(vultr-cli server list | grep -vE '^ID' | sed -n '/^===/q;p')
	...
	[[ ${input} == 'YES' ]] && vultr-cli server delete ${instance_id}
fi
...
then
	...
	[[ -n ${instance_id} ]] && vultr-cli server restart ${instance_id}
fi
...
then
	...
	[[ -n ${instance_id} ]] && vultr-cli server stop ${instance_id}
fi
...
then
	...
	[[ -n ${instance_id} ]] && vultr-cli server start ${instance_id}
fi
